...
exports.register = async (req, res) => {
  try {
    const { username, email, password, first_name, last_name, user_type, bio } =
      req.body;
    ...
    const token = jwt.sign(
      { userId: userId, email: email, user_type: user_type },
    ...
    res
      .status(201)
      .json({ userId, token, message: "User registered successfully" });
  } catch (error) {
